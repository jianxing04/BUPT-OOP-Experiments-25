...
set<int> userLoginSet; // 用于存储已登录用户的ID，防止重复登录
...
public:
    ...
    void registerAccount(const json& request, json& response)const {
		...
		newUser["message"] = json::array(); // 初始化消息列表
        ...
        for (const auto& user : allUsers) {
			if (user["username"] == username) {
				if (user["password"] == password) {
					if (userLoginSet.find(user["id"].get<int>()) != userLoginSet.end()) {
						response["status"] = "error";
						response["message"] = "User already logged in.";
						return;
					}
					userLoginSet.insert(user["id"].get<int>());
					response["status"] = "success";
	...
	// 用户登出
	void userLogout(const json& request, json& response)const {
		string username = request["username"];
		for (const auto& user : allUsers) {
			if (user["username"] == username) {
				int userID = user["id"].get<int>();
				if (userLoginSet.find(userID) != userLoginSet.end()) {
					userLoginSet.erase(userID);
					response["status"] = "success";
    ...
    void viewMyShoppingTrolley(const json& request, json& response)const {
        ...
        for (const auto& user : allUsers) {
            if (user["username"] == username) {
				if (user["shoppingtrolley"].empty()) {
                    response["status"] = "error";
					response["message"] = "Shopping trolley is empty.";
					return;
				}
				response["status"] = "success";
				for (const auto& item : user["shoppingtrolley"]) {
	...
	void payForTheOrder(const json& request, json& response)const {
		...
		for (auto& user : allUsers) {
			if (user["username"] == username) {
				if (user["orders"].empty()) {
					response["status"] = "error";
					response["message"] = "No orders to pay for.";
					return;
				}
				int cnt = 0;
				for (auto it = user["orders"].begin(); it != user["orders"].end(); ) {
					if (request["choices"][cnt].get<char>() == 'n') {
						cnt++;
						it++;
						continue;
					}
					for (const auto& item : (*it)) {
                        ...
                        for (auto& merchant : allUsers) {
							if (merchant["id"] == ownerID) {
								...
								string message = "您收到订单：" + item["name"].get<string>()
									+ "，商品id：" + to_string(item["id"].get<int>())
									+ "，一共" + to_string(item["count"].get<int>()) + "件";
	...
	void denyMyOrders(const json& request, json& response)const {
		...
		for (auto& user : allUsers) {
			if (user["username"] == username) {
				if (user["orders"].empty()) {
					response["status"] = "error";
					response["message"] = "No orders to cancel.";
					return;
				}
				int cnt = 0;
				for (auto it = user["orders"].begin(); it != user["orders"].end(); ) {
	...
	void discountByCategory(const json& request, json& response)const {
		...
		if (flg == 0) {
			response["status"] = "error";
			response["message"] = "No merchandise found in this category for the owner.";
			return;
		}
		response["status"] = "success";
	...
	void viewMyMerchandise(const json& request, json& response)const {
		...
		if (flg == 0) {
			response["status"] = "error";
			response["message"] = "No merchandise found for this owner.";
			return;
		}
	}
	// 查看我的消息
	void viewMyMessage(const json& request, json& response)const {
		string username = request["username"];
		for (const auto& user : allUsers) {
			if (user["username"] == username) {
    ...
    void handleRequest(SOCKET clientSocket) {
        try {
            ...
            while (true) {
				...
				else if (action == "viewMyMessage") {
					viewMyMessage(request, response);
				}
                else {
                    response["status"] = "error";
